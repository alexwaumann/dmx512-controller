...
void init_uart_dmx( void )
{
    ...
    GPIO_PORTC_AFSEL_R |= 32;                   // set 6th bit to enable peripheral control for PC5
    GPIO_PORTC_PCTL_R  |= GPIO_CPTL_PC5_U1TX;   // UART1 TX ON PC5
    ...
    UART1_CTL_R  &= ~UART_CTL_UARTEN;           // disable UART0 for configuration
...
void Timer1ISR(void){
    if(dmx512_state == 0){
        ...
        UART1_CTL_R  |= UART_CTL_UARTEN             // enable UART1 and TX
                     |  UART_CTL_TXE;
    }
